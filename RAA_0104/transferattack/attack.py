...
class Attack(object):
    ...
    def forward(self, data, label, **kwargs):
        ...
        return delta.detach(), self.model
    
    def feat_forward(self, data, feats, **kwargs):

        data = transforms.ToTensor()(data)
        data = data.clone().detach().to(self.device)
        feats = torch.tensor(feats)
        feats = feats.clone().detach().to(self.device)
        self.model.eval()
        ...
        for _ in range(self.epoch):
            delta.requires_grad = True
            # Obtain the output
            x = (delta+data).unsqueeze(0)
            
            logits = self.get_logits(self.transform(x, momentum=momentum))

            # Calculate the loss
            loss = self.get_feature_loss(logits, feats)

            # Calculate the gradients
            grad = self.get_grad(loss, delta).unsqueeze(0)
        ...
        return delta.unsqueeze(0).detach(), self.model
    ...
    def get_feature_loss(self, logits, feats):
        """
        The loss calculation, which should be overrideen when the attack change the loss calculation (e.g., ATA, etc.)
        """
        # Calculate the loss
        return -self.loss(logits, feats) if self.targeted else self.loss(logits, feats)
